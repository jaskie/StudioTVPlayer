...
namespace StudioTVPlayer.ViewModel.Configuration
{
    public class FoldersViewModel : ModifyableViewModelBase
    {
        ...
        private void AddWatchedFolder(object obj)
        {
            ...
            {
                var name = path.Split([System.IO.Path.DirectorySeparatorChar], StringSplitOptions.RemoveEmptyEntries).LastOrDefault() ?? "New watched folder";
        ...
        {
            var folder = sender as WatchedFolderViewModel ?? throw new ArgumentException($"{nameof(WatchedFolderViewModel)} expected, {sender?.GetType()} got.");
