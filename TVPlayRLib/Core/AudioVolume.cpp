...
std::vector<double> AudioVolume::ProcessVolume(const std::shared_ptr<AVFrame>& frame)
{
	...
	std::vector<int64_t> peak_volume_int64(frame->channels, 0LL);
	for (int i = 0; i < samples_count; i++)
	{
		if (volume_ != new_volume_ && i > 0 && (samples[i] * samples[i - 1]) < 0) // sign of sample was changed, probably near zero, we can now adjust the volume
			volume_ = new_volume_;
		samples[i] = av_clipl_int32((((int64_t)samples[i] * volume_ / ZERO)));
	...
	std::transform(peak_volume_int64.begin(), peak_volume_int64.end(), peak_volume_double.begin(), [](int64_t volume) { return static_cast<double>(volume) / (std::numeric_limits<int32_t>().max)(); });
