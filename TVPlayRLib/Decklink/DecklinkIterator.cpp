...
namespace TVPlayR {
	namespace Decklink {
		...
		struct DecklinkIterator::implementation {
			...
			std::shared_ptr<DecklinkOutput> CreateOutput(const DecklinkInfo& info, DecklinkKeyer keyer)
			{
				return std::make_shared<DecklinkOutput>(info.GetDecklink(), keyer, info.Index());
			}
		...
		std::shared_ptr<DecklinkOutput> DecklinkIterator::CreateOutput(const DecklinkInfo& info, DecklinkKeyer keyer) { return impl_->CreateOutput(info, keyer); }
