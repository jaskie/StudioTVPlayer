...
class AudioFilter::implementation
{
private:
...
public:
	implementation(const std::vector<std::unique_ptr<Decoder>>& decoders, int64_t output_channel_layout, const AVSampleFormat sample_format, const int sample_rate, const int nb_channels)
		: decoders_(decoders)
		, time_base_(av_make_q(1, sample_rate))
		, nb_channels_(nb_channels)
		, fifo_(sample_format, nb_channels, sample_rate, time_base_, 0LL, 2048)
		, output_channel_layout_(output_channel_layout)
		, sample_format_(sample_format)
		, graph_(nullptr, [](AVFilterGraph * g) { avfilter_graph_free(&g); })

	{
		filter_str_ = GetAudioFilterString(sample_rate);
		CreateFilterChain();
	}

	std::string GetAudioFilterString(const int sample_rate)
	{
		std::ostringstream filter;
		int total_nb_channels = std::accumulate(decoders_.begin(), decoders_.end(), 0, [](int sum, const std::unique_ptr<Decoder>& curr) { return sum + curr->channels_count; });
		if (decoders_.size() > 1)
		{
	...
	void Push(int stream_index, AVFramePtr frame)
	{
		...
		if (dest == std::end(source_ctx_))
			return;
		av_buffersrc_write_frame(dest->second, frame.get());
		auto received_frame = AllocFrame();
		if (FF(av_buffersink_get_frame(sink_ctx_, received_frame.get())))
			fifo_.TryPush(received_frame);
	}

	AVFramePtr Pull(int audio_samples_count)
	{
		std::lock_guard<std::mutex> guard(mutex_);
		return fifo_.Pull(audio_samples_count);
	}
	...
	void CreateFilterChain()
	{
		if (decoders_.empty())
			return;
		if (std::find_if(decoders_.begin(), decoders_.end(), [](const std::unique_ptr<Decoder>& decoder) -> bool { return decoder->ctx->codec_type != AVMEDIA_TYPE_AUDIO; }) != decoders_.end())
			throw Common::TVPlayRException("AudioFilter::CreateFilterChain() got non-audio stream");
		...
		int out_sample_rates[] = { decoders_[0]->ctx->sample_rate, -1 };
		...
		try
		{
			...
			for (int i = 0; i < decoders_.size(); i++)
			{
				auto channel_layout = decoders_[i]->ctx->channel_layout;
				if (!channel_layout)
					channel_layout = av_get_default_channel_layout(decoders_[i]->ctx->channels);
				snprintf(args, sizeof(args),
					"time_base=%d/%d:sample_rate=%d:sample_fmt=%s:channel_layout=0x%llx",
					decoders_[i]->stream->time_base.num, decoders_[i]->stream->time_base.den, decoders_[i]->ctx->sample_rate,
					av_get_sample_fmt_name(decoders_[i]->ctx->sample_fmt), channel_layout);
				auto new_source = std::pair<int, AVFilterContext*>(decoders_[i]->stream_index, NULL);
			...
			if (avfilter_graph_parse_ptr(graph_.get(), filter_str_.c_str(), &inputs, &outputs, NULL) < 0)
				throw std::exception("avfilter_graph_parse_ptr failed");
			if (avfilter_graph_config(graph_.get(), NULL) < 0)
				throw std::exception("avfilter_graph_config failed");
#ifdef DEBUG
...
AudioFilter::AudioFilter(const std::vector<std::unique_ptr<Decoder>>& decoders, int64_t output_channel_layout, const AVSampleFormat sample_format, const int sample_rate, const int nb_channels)
	: impl_(new implementation(decoders, output_channel_layout, sample_format, sample_rate, nb_channels))
{ }

AudioFilter::~AudioFilter() { }

int AudioFilter::OutputSampleRate() { return impl_->GetSampleRate(); }
int AudioFilter::OutputChannelsCount() { return impl_->GetChannelsCount(); }
AVRational AudioFilter::OutputTimeBase() const { return impl_->GetTimeBase(); }
uint64_t AudioFilter::OutputChannelLayout() { return impl_->GetChannelLayout(); }
AVSampleFormat AudioFilter::OutputSampleFormat() { return impl_->GetSampleFormat(); }
void AudioFilter::Push(int stream_index, AVFramePtr frame) { impl_->Push(stream_index, frame); }
AVFramePtr AudioFilter::Pull(int audio_samples_count) { return impl_->Pull(audio_samples_count); }
void AudioFilter::Flush() { impl_->Flush(); }
void AudioFilter::Reset() { impl_->Reset(); }
bool AudioFilter::IsEof() const { return impl_->IsEof(); }
