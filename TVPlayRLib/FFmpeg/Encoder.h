...
namespace TVPlayR {
	...
	private:
		...
		std::deque<std::shared_ptr<const AVFrame>> frame_buffer_;
		const int format_;
		bool is_eof_ = false;
		...
		std::unique_ptr<AVCodecContext, std::function<void(AVCodecContext*)>> GetVideoContext(AVFormatContext* const format_context, const AVCodec* encoder, int bitrate, AVPixelFormat pixel_format, int width, int height, AVRational time_base, AVRational frame_rate, AVRational sar, bool interlaced);
		void OpenCodec(AVFormatContext* const format_context, AVDictionary** options, const std::string& stream_metadata, int stream_id);
		bool InternalPush(const AVFrame* frame);
		std::shared_ptr<AVFrame> GetFrameFromFifo(int nb_samples);
	public:
		Encoder(const OutputFormat& output_format, const AVCodec* encoder, int bitrate, AVPixelFormat pixel_format, std::shared_ptr<const AVFrame> video_frame, AVRational time_base, AVRational frame_rate, AVDictionary** options, const std::string& stream_metadata, int stream_id);
		...
		void Push(const std::shared_ptr<const AVFrame>& frame);
		void Flush();
