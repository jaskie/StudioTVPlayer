...
public:
	...
	bool Push(const std::shared_ptr<const AVFrame> frame, AVRational input_frame_rate, AVRational input_time_base);
private:
	const Core::VideoFormat output_format_;
	const AVPixelFormat output_pixel_format_;
	std::string GetFilterString(const std::shared_ptr<const AVFrame>& frame, Common::Rational<int> input_frame_rate);
};
